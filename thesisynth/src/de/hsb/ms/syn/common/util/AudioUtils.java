...
/**
 * Utility class for specific audio things.
 * It contains the generation algorithms for most Gen Nodes, for example
 * @author Marcel
 *
 */
public abstract class AudioUtils {
	...
	public static float[] sinewave(float freq, float velocity) {
		...
		for (int i = 0; i < samples; i++) {
			vals[i] = velocity * (float) Math.sin(2 * Math.PI * i / (44100/freq));
		}
		return vals;
	}
	...
	public static float[] sawtooth(float freq, float velocity) {
		...
		int samples = (int) Math.floor(Constants.SAMPLING_RATE * (1/freq));
		float[] vals = new float[samples];
		// Use the linear function f(x) = 2/samples * x - 1 for emulating the sawtooth
		for (int i = 0; i < (vals.length - (samples % 2)) / 2; i++) {
	...
	/**
	 * Convert the given time in milliseconds to an amount of samples with the same duration
	 * (for instance, at 44100 kHz, the number of samples for 500 ms would be 22050)
	 * @param time
	 * @return
	 */
	public static int timeToSamples(int time) {
		return Math.round((float) Constants.SAMPLING_RATE * ((float) time / 1000.0f));
	}
