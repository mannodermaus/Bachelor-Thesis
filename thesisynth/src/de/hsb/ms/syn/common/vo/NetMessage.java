...
/**
 * Value object wrapper for messages that are sent over network.
 * Every NetMessage has got a String message and may optionally
 * have extra data (Map format) to be accessed.
 * (This implementation is similar to Android's Bundle class)
 * @author Marcel
 *
 */
public class NetMessage implements Serializable {
	...
	/**
	 * Returns the Map's keySet in order to access the
	 * NetMessage's contents, or null if it doesn't contain any
	 * @return
	 */
	...
	public void addExtra(String key, Serializable value) {
		// Create the Map in the first place if this is the first Extra to be stored
		...
		if (key.equals(NetMessages.EXTRA_ARGS)) {
			...
			// (if more than one argument are passed in via addExtra(Messages.EXTRA_ARGS),
